...
\begin{table}[]
  \caption{Summary of models; their performance, F-statistics and Residual error.}
      ...
      \resizebox{\textwidth}{!}{\begin{tabular}{@{}cccccccc@{}}
        \toprule
          & \textbf{$R^2$}       & \textbf{$Adjusted R^2$} & \textbf{F-statistic} & \textbf{P-Value}     & \textbf{\begin{tabular}[c]{@{}c@{}}Residual\\ Standard Error\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Residual Sum\\ of Squares\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Residual Mean\\ of Squares\end{tabular}} \\ \midrule
...
% \ref{tab:ftests}
% We need a table with all F-tests.
%         | Yield   | ...
% Model 1 | -2e^-16 | ...
%
To determine if predictors significantly related to a Model's response variable, F-tests were conducted. Models 1 and 2 showed significant linear relationships present between all predictors and their response variables (see Table \ref{tab:ftests}). Predictors in Models 3 and 4 reported the same significance for each variable except for scope 1 emissions. Scope 1 emissions was included in all models to directly compare the response variables as ratios of vineyard size to raw values. Even though not significant within models 3 and 4, when using the Pearson Correlation Coefficients scope one emissions was strongly correlated to every Model's response variable; this was especially so for Model 1 and 4 (Yield and average price per tonne as a ratio to area harvested, respectively).
