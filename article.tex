...
% The tables need to be in the appropriate sections!!
%
%Where are the coefficients???? You cant use the models without them!!
% You need to actually be able to use this research and look at what these
% models are showing.
%
...
% You can draw similarities between the model, as this is not a generalisation.
%
%
%
...
\newline
% F-tests
To determine if individually each model's predictors were significantly linearly related to the response variables, F-tests were conducted. Models 1 and 2 showed significant linear relationships between each of the predictors and their response variables (see Tables 6 and 7). Predictors in models 3 and 4 reported similar significance; except for scope 1 emissions (see Tables 8 and 9). Scope one emissions was included in all models to directly compare the response variables as ratios of vineyard size to raw values. Even though not significant within models 3 and 4, when using the Pearson Correlation Coefficients, scope one emissions was strongly correlated to every Model's response variable; this was especially so for Model 1 and 4 (Yield and average price per tonne as a ratio to area harvested, respectively).
\newline
% t-tests
Predictors were also reviewed using t-tests to determine if a predictor significantly contributed to the model when accounting for other variables. We can consider this the marginal significance of each variable. Using t-tests allowed a more granular examination of each factor (or categorical variable) and interaction, showing that some years and areas contributed marginally significantly and some did not (see appendix). With regard to continuous variables, Model 1 and 2 showed water use, scope 1 emissions and area harvested were marginally significant. For Model 3 all continuous variables except scope 1 emissions were significant.Model 4 showed scope 1 emissions and water use to only be significant when considered as an interaction with area harvested but not as individual variables (see Table ?).
