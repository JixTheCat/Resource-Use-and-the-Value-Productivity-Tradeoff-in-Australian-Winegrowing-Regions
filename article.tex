...
The coefficients of each model describe the relationship of a predictor variable to its response when considering all other predictor variables. Due to the transformations of the data, coefficients are individually interpreted in the same manner as the prior regression values were (section\ref{sec:exp_anal}); although all coefficients need to be considered together. Unlike the regression values the coefficients range is not limited between -1 and 1, as the ratio of each variables contribution needs to be considered together.
...
\newline
We look at the categorical variables' and continuous variables' coefficients separately. This is primarily done as the categorical variables have many coefficients, one for each category, whilst continuous variables have only one. The coefficient for categorical variables are summarised in Figure\ref{fig:violin}. The continuous variables coefficients are summarised in Table\ref{tab:contvar}.
\begin{figure}
...
%
\begin{table}[]
  \caption{Summary of models; their performance, F-statistics and Residual error.}\label{tab:contvar}
      \resizebox{\textwidth}{!}{
